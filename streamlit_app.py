...
"""
get_stream: takes a sentence and breaks it down into words and yields them one by one with a small delay between them to create a streaming effect.
...
# CORE: Function to convert a sentence to stream
...
# CORE: transforms datafrme into csv string
def get_data(placeholder):
    new_data = st.session_state.df.to_csv()
    return new_data


# CORE: creates dataframe within UI
def get_dataframe():
    df = pd.DataFrame(
        {
...
# CORE CHATBOT LOGIC: call OpenAI for a report tool, if used -> makes plan and streams it, otherwise streams normal reply
...
"\U0001F600"
# CORE: run the plan generated by the chatbot and update session state
...
# CORE: convert agent steps into .md 
...
# CORE: takes .md and display it in the UI
...
# CORE: make dynamic UI update on streamlit that stores values in session state and rerun script for it to show in code tab
class SessionStateAutoClass:
    '''both special methods used at end to set formatted steps and rerun to see new plan'''
    # intercept attribute to modify and rerun session
    def __setattr__(self, name, value):
        if getattr(self, name, None) != value:
            st.session_state[name] = value
            st.rerun()
    # allow display of changed attr when attr is not found
