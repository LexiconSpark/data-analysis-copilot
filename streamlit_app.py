...
def add_new_tab():
    new_tab_name = f"Sheet{len(st.session_state.tabs)}"
    st.session_state.tabs.insert(-1, new_tab_name)
    st.session_state[f"df_{new_tab_name}"] = get_dataframe()

def add_import_table(new_tab_name, data):
    st.session_state.tabs.insert(-1, new_tab_name)
    st.session_state[f"df_{new_tab_name}"] = data

def rename_tab(index, new_name):
    st.session_state.tabs[index] = new_name

def generate_pdf(content, file_path):
    c = canvas.Canvas(file_path, pagesize=letter)
    width, height = letter
    c.drawString(100, height - 100, "AI Generated Report")
...
if "tabs" not in st.session_state:
    st.session_state.tabs = ["Sheet1", "New One"]
    st.session_state["df_Sheet1"] = get_dataframe()
                ...
                if tab_name == "New One":
                    with selected_tab[i]:
                        ...
                        if st.button("Create New Sheet"):
                            add_new_tab()
                            st.rerun()
                        uploaded_files = st.file_uploader("Import one or several CSV files", accept_multiple_files=True)
                        if uploaded_files:
                            for uploaded_file in uploaded_files:
                                data = pd.read_csv(uploaded_file)
                                st.write("filename:", uploaded_file.name)
                                add_import_table(uploaded_file.name,data)
                                st.rerun()

                else:
                    with selected_tab[i]:
                        st.header(tab_name)
                        new_name = st.text_input(f"Rename {tab_name}", tab_name, key=f"rename_{tab_name}")
                        if new_name != tab_name:
                            rename_tab(i, new_name)
                            st.rerun()
                        
                        # if the dataframe is not in the session state
                        if f"df_{tab_name}" not in st.session_state:
                            st.session_state[f"df_{tab_name}"] = get_dataframe()

                        # display the editable table
                        edited_df = st.data_editor(
                             st.session_state[f"df_{tab_name}"],
                            key=f"editable_table_{tab_name}",
                            num_rows="dynamic",
                            on_change=handle_table_change,
                        )

                        # update the dataframe
                        st.session_state[f"df_{tab_name}"] = edited_df
   ...
   # create the fourth column in second row
    with col2row2:
        with st.container(height=ROW_HIGHT):
            ...
            # execute the code
            exec(reporting_code)
            # Add a download button to export the report as PDF
